...
OBJ_F = 'myobj';   % Name of function for objective values
...
% compute SUBPOP, NIND depending on number of variables (defined in objective function)
   ...
   MAXGEN = 100 * floor(sqrt(NVAR)); % Max number of generations
...
% Plot Results and show best individuals => optimum
   figure('Name',['Results of ' FigTitle num2str(min(Results(:,3)))]);
  % subplot(2,1,1), plot(Results(:,1),Results(:,2),'-',Results(:,1),Results(:,3),':');
 x=(1:1:size(Results(:,3)))';
  %subplot(2,1,1), plot(x,Results(:,2),'-',x,Results(:,3),':');
  subplot(2,1,1), plot(x,Results(:,3),'-');
   subplot(2,1,2), plot(IndAll(gen-4:gen,:)');
    ...
    save('./results_paras/paras.mat','nn','aa','bb');
